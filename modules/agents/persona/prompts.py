...
def load_persona_prompts():
    """
    Load the Persona Agent prompts from YAML definition.

    Expects a YAML file with keys:
      - system: system message template
      - user: user message template with placeholder {idea}
    """
    # Resolve path to project root (three levels up)
    base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))

    prompts_path = os.path.join(base_dir, 'prompts', 'persona_agent.yml')
